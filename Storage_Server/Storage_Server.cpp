// Storage_Server.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
void ProcessClient(SOCKET hSock)
{
    ...
    cout << m.getHeader().msgFrom << ": " << typesNames[typeCode]<<" TO";
    switch (m.getHeader().msgTo)
    {
    case -1: {
        cout << " ALL_USERS" << endl;
        break;
    ...
    default: {
        cout << " " << m.getHeader().msgTo << " this data: " << m.getData() << endl;
        break;
    }
    }
    switch (typeCode)
    ...
    case GETALLDATA:
    {
        ...
        {
            sessions[m.getHeader().msgTo]->senthist(s);
        }
        break;
    }
    default:
        if (sessions.find(m.getHeader().msgFrom) != sessions.end())
        {
            if (sessions.find(m.getHeader().msgTo) != sessions.end())
            {
                ...
                sessions[m.getHeader().msgFrom]->Add(m);
            }
